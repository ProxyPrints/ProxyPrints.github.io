...
describe("buildDisplayPDFProps - rail guide state reaches the exported PDF's cut lines", () => {
  it("showCutLines: true -> drawCardCutLines: true (and the rail's lime corner-only geometry)", () => {
    const props = buildDisplayPDFProps(baseInput);
    expect(props.drawCardCutLines).toBe(true);
    // Geometry defaults named in the adapter match PagePreview's E19 lime corner-only guides, so
    // the exported guides look like the guides the sheet showed.
...
describe("buildDisplayPDFProps - named defaults for fields with no editor equivalent", () => {
  it("exports at full resolution, 600 DPI, 100% JPEG quality", () => {
    const props = buildDisplayPDFProps(baseInput);
    expect(props.imageQuality).toBe("full-resolution");
    expect(props.imageDPI).toBe(600);
    expect(props.jpgQuality).toBe(100);
  });

  it("selection mode defaults to fronts + distinct backs (the rail's face toggle is a view, not an export filter)", () => {
    expect(buildDisplayPDFProps(baseInput).cardSelectionMode).toBe(
