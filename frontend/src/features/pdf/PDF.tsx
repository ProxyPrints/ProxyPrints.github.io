...
const CutLineCorner = ({
  ...
  // The cut line marks the TRUE card edge, which is `bleedMM.<edge>` in from this slot's own
  // boundary, not a flat `bleedEdgeMM` - left/right (the column axis) and top/bottom (the row
  // axis) can legitimately differ (one axis full bleed, the other cropped - see layout.ts's
  // fitAxisWithBleed), so this needs two separate offsets, not one uniform offset.
