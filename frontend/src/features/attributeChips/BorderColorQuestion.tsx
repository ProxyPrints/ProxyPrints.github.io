...
// follow-up, while a border question needs exactly the four BORDER_COLOR_GROUP chips (§5 rule
// 1: "render only what the current question needs"). The vote machinery is shared, not forked -
// these chips cast the same CardTagVote through the same APISubmitTagVote "question-feed"
// surface every other WTC chip casts (useTagVoting), so a border answer is a first-class vote
// on an existing axis, not a new vote model (additive-only scope: no schema/endpoint changes).
