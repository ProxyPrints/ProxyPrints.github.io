...
class Command(BaseCommand):
    help = (
        "Casts the pre-existing 'Extended' attribute chip from the art-edge continuity classifier "
        "(cardpicker.local_art_edge - a self-referential within-image colour comparison) by "
        "reading Stage C's already-persisted ImageEvidence.art_edge_class - zero image fetches. "
        "Positive-only: votes only when the stored reading is 'extended'; 'framed'/'mixed' abstain. "
        "Never resolves a tag by itself: a single VoteSource.OCR vote cannot clear the "
        "human-backed gate (vote_consensus.resolve_weighted_consensus). Defaults to dry-run and "
        "requires an explicit --write, matching every other Stage 3+ command's own convention."
    )
    ...
    def handle(self, *args: Any, **kwargs: Any) -> None:
        ...
        print(f"[{mode}] local_art_edge_cast run_id={run_id} git_sha={get_baked_git_sha()}")

        ledger = PilotRunLedger.objects.create(
            run_id=run_id,
            command="local_art_edge_cast",
            dry_run=dry_run,
        ...
        try:
            result = run_art_edge_continuity_cast(run_id=run_id, dry_run=dry_run, chunk_size=kwargs["chunk_size"])
            print(
                f"[art-edge-continuity] considered={result.cards_considered} "
            ...
            if not dry_run:
                touched_card_ids = list(
                    CardTagVote.objects.filter(run_id=run_id, anonymous_id=ART_EDGE_ANONYMOUS_ID).values_list(
                        "card_id", flat=True
                    )
                )
