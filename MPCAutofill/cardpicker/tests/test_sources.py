...
class TestAPI:
    ...
    def test_image_language(self, django_settings, image, expected_language):
        tags = Tags()
        if expected_language is None:
            language, _, _, _, _, _, _, _, _ = image.unpack_name(tags=tags)
            assert language is None
        else:
            language, _, _, _, _, _, _, _, _ = image.unpack_name(tags=tags)
            assert language.alpha_2.lower() == expected_language.lower()
    ...
    def test_image_tags(self, django_settings, grandchild_tag, extended_tag, full_art_tag, image, expected_tags):
        tags = Tags()
        _, _, extracted_tags, _, _, _, _, _, _ = image.unpack_name(tags=tags)
        assert extracted_tags == expected_tags
    ...
    def test_unpack_name(
        ...
        tags = Tags()
        language, name, extracted_tags, extension, _, _, _, _, _ = image.unpack_name(tags=tags)
        if expected_language is None:
            assert language is None
        else:
            assert language.alpha_2.lower() == expected_language.lower()
...
class TestUpdateDatabase:
    ...
    def test_transform_image_into_object_stamps_md5_checksum_from_listing(self, django_settings):
        """Issue #473 PR-1: Card.md5_checksum is copied verbatim from Image.md5_checksum at the
        exact seam where a Card is first built from a source's listing - no separate fetch."""
        source = factories.SourceFactory()
        image = Image(
            id="x",
            name="Card.png",
    ...
    def test_transform_image_into_object_leaves_md5_checksum_null_when_listing_has_none(self, django_settings):
        """Never invented - a LOCAL_FILE source's Image (or any Drive listing entry that
        genuinely omits md5Checksum) has md5_checksum=None, and that's exactly what lands on the
        Card too (Card.md5_checksum's own null-is-a-singleton-group contract, issue #473 ruling
        3)."""
        source = factories.SourceFactory()
        image = Image(
            id="x",
        ...
        bulk_sync_objects(
            ...
            cards=[
                Card(
                    ...
                    searchq="mountain",
                    date_created=make_aware(DEFAULT_DATE),
                    date_modified=make_aware(DEFAULT_DATE),  # unchanged
                    source=source,
                    tags=[],
                    size=0,
    ...
    def test_associate_with_canonical_artist(
        ...
        _, _, _, _, canonical_card_id, canonical_artist_id, _, _, _ = Image(
            id="",
            name=new_card,
            size=0,
            created_time=dt.datetime(2026, 1, 1),
