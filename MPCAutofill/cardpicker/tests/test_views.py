...
# `factory.Sequence` counters are process-global for the whole pytest run - unpinned, this
# module's snapshots (`__snapshots__/test_views.ambr`) would embed sequence-derived values (e.g.
# an autogenerated "Artist N" name, via `brainstorm_canonical_card`'s default
# `CanonicalCardFactory`/`CanonicalArtistFactory` SubFactory chain) that silently depend on total
# call count up to that point in collection order - both across the whole suite AND across this
# file's own earlier tests. Rather than tracking that ambient count (the old approach: every
# *other* file using these shared factories had to capture/restore its own use of them around
# itself so this file's hardcoded values stayed put - fragile, forgotten repeatedly, see
# docs/troubleshooting.md's "5-6 unrelated test snapshots break" entry), this module pins the
# shared factories to a fixed baseline before every one of its own tests. That makes this file's
# output fully self-determined - independent of suite composition, collection order, or how many
# tests ran before it - so no capture/restore dance and no per-test insulation list are needed
# anymore, here or in any other file in this package.
_SHARED_FACTORIES = [
    CardFactory,
    SourceFactory,
    CanonicalArtistFactory,
...
def _pin_shared_factory_sequences():
    for f in _SHARED_FACTORIES:
