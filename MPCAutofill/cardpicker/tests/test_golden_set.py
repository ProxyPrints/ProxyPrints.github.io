...
EXTRACTORS_WITHOUT_GOLDEN_EXPECTATIONS = {
    ...
    "artbox_phash",
    # PROVISIONAL, same reasoning as artbox_phash above. canvas_padding writes one pinnable
    # discrete value (`canvas_padding_verdict`: padded/not_padded/abstain_black/ambiguous - no
    # different in kind from `geometry_bleed`'s `bleed_class` or `layout_class`'s own output,
    # both of which ARE pinned) alongside four continuous per-edge fractions and four discrete
    # per-edge calls that golden_set.py would exclude on the same "continuous/brittle" grounds
    # `fetch_latency_ms`/width/height already are. It is pinnable and simply has not been pinned;
    # closing it needs the same real 30-card host-venv run with prod credentials artbox_phash's
    # waiver defers.
    "canvas_padding",
}
