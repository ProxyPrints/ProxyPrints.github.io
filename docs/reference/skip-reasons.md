...
| `border-mismatch`         | `JOIN_KEY_BORDER_MISMATCH_SKIP_REASON`         | The corroboration layer: observed border colour contradicts the matched printing's own border value.                                                                               | **Retired 2026-08-14.** Nothing writes it; kept declared and kept in `JOIN_KEY_NO_HIT_SKIP_REASONS` so historical rows still route to review. A border disagreement now weakens confidence (`JOIN_KEY_CONFIDENCE_ARTIST_DISAGREEMENT`) instead of vetoing the match. |
