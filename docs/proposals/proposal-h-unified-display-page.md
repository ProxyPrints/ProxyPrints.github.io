...
existing `GridSelectorModal` verbatim; see §5's row and §6 step 4).
...
four further confirmed gaps into this doc's body, one still open —
**export surfaces beyond PDF** (§5, `ExportXML.tsx`/`ExportImages.tsx`/
...
for this one's sequencing. A related but deliberately
...
4. **Toolbar instrument parity (issues #239, #240, #241)** — the other
   ...
   (`CommonCardback.tsx`'s new `CardbackToolbarButton` export, unchanged
   modal — see §5's note on why this one keeps its own modal rather than
   following the Choose Image "no modal" exception, issue #240 — **DONE**,
   mounted in `DisplayPage.tsx`'s toolbar); and an Export ▾ dropdown
   composing `ExportXML`/`ExportImages`/`ExportDecklist` alongside the
   existing Generate PDF / Save to Drive buttons (issue #241). See §5 for
