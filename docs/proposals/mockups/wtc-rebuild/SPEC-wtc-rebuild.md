...
| **L3 · Follow-up**           | open exclusion groups only                         | any candidate-type post-pick                             | Level 3 (L22–29): tri-state chips (`level3ChipStates`) cast immediately; Confirm & continue / Skip; groups = `BORDER_COLOR_GROUP`/`FRAME_STYLE_GROUP`/`FRAME_TREATMENT_GROUP` (Full Art/Borderless/Showcase/Extended, now one mutually-exclusive axis - DESIGN-REPASS-2026-08.md Rule 7) from attributeChips.ts; Border Color is never "open" for a candidate Frame Treatment already resolves to Borderless/Full Art | appears only after a pick; "one more thing" framing                                                                    |
