...
| **L3 · Follow-up**           | open exclusion groups only                         | any candidate-type post-pick                             | Level 3 (L22–29): tri-state chips (`level3ChipStates`) cast immediately; Confirm & continue / Skip; groups = `BORDER_COLOR_GROUP` (Black/White/Silver/Borderless)/`FRAME_STYLE_GROUP`/`FRAME_TREATMENT_GROUP` (Showcase/Extended Art, the one measured-exclusive pair - DESIGN-REPASS-2026-08.md Rule 7) from attributeChips.ts; Full Art is standalone and independent of every group; Border Color is never "open" for a candidate whose own Borderless chip already resolves it | appears only after a pick; "one more thing" framing                                                                    |

Frame Treatment's mutual exclusivity (DESIGN-REPASS-2026-08.md Rule 7) is a presentation/
asking-order change only - which chips render and when Border Color is asked. The vote/tag
data model, `CardTagVote`, and consensus computation are unchanged; no new vote types exist.
