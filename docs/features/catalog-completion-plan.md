...
**`compute_color_profile` retired — `extraction-soundness-batch` (2026-07-27)**: `compute_color_profile` in `local_image_quality.py` and its corresponding `color_profile` extractor block in `image_evidence.py` were removed. `color_mean_rgb`/`color_stddev_rgb` fields on `ImageEvidence` are kept (existing DB rows untouched; no migration needed). `COLOR_PROFILE_EXTRACTOR_VERSION` constant and its `MANIFEST_EXTRACTOR_KEYS` entry removed. All `color_profile` tests and golden expectations removed. Rationale: per-channel mean/stddev values were documented as documentation artifacts only (see golden-set note above, "its real recorded per-card mean/stddev values are kept in `golden_set.py` as a documentation artifact only"), never consumed by any downstream Stage D step or operator tooling, and the extractor added non-trivial per-card extraction time for zero pipeline benefit.
...
**FOLLOW-UP — the version bump alone stales without erasing (same issue #925, same day)**:
`persist_evidence` only writes keys present in `result.fields`, and the abstain branch never added
`artbox_phash`/`artbox_crop_px` to `fields` - so the bump above re-tags a re-extracted row's
existing pre-abstain hash as `artbox-phash-v2` without ever removing it, which is worse than the
original tag confusion: the `artbox_frame_class=""`-with-populated-hash signature that currently
identifies the 8,534 tainted rows would disappear while the wrong hash remained. Fixed by having
both `EXTRACTOR_AMBIGUOUS_SKIP_REASON` branches (unclassifiable frame, degenerate crop box) set
`fields["artbox_phash"] = None` and `fields["artbox_crop_px"] = None` explicitly, so
`persist_evidence` clears the stored value instead of merely leaving it untouched. The
`EXTRACTOR_FETCH_FAILED_SKIP_REASON` branch (`image is None`) is deliberately unchanged: the
extractor never ran, so a transient fetch failure carries the existing value forward exactly as
before, the same as every other extractor's fetch-failure path. Tests assert on the persisted
value directly (`test_persist_clears_a_pre_abstain_hash_when_reextraction_is_unclassifiable`,
`test_persist_keeps_the_existing_hash_when_reextraction_cannot_fetch` in `test_image_evidence.py`),
not just on re-extraction eligibility. Measured read-only against production 2026-08-27: 8,534
rows match `artbox_frame_class="" AND artbox_phash IS NOT NULL` today - once the version-bump
above's incremental pass runs, this fix is what makes all 8,534 end with `artbox_phash`/
`artbox_crop_px` NULL rather than a re-stamped guess. Still code-only: no backfill or
re-extraction run against production.
...
**CORRECTION (2026-07-22) — the memory-budget arithmetic above assumed a bound the
implementation never enforced, and it leaked in production.** The "worst-case buffers alive at
once ≈ fetch threads in flight + queue depth + one per compute worker ≈ 31" arithmetic a few
paragraphs up silently assumed fetch COMPLETION was bounded by `queue_depth` the same way compute
submission was — the shipped `_run_cohort` never actually enforced that: it submitted every
`cohort_ids` entry to the fetch thread pool up front and only gated compute submission behind
`queue_depth`. Since fetch (paced by `GOOGLE_IMAGE`'s 6-way concurrency limiter) completes cards
materially faster than the 7-process CPU-bound compute stage consumes them, fetch raced
arbitrarily far ahead of consumption in a real multi-hour run — every completed-but-not-yet-
consumed fetch result retains its raw image buffer, so the buffer count actually scaled with total
cards fetched-so-far, not with the small constant this arithmetic assumed. Two production
incidents on the 197,428-card remainder run surfaced this: an unwatched run OOM-killed the whole
box overnight at ~21GB parent RSS; a re-run the following night was caught by a watchdog and
stopped cleanly at 17.3GB after 66,355 cards (~250-350KB retained per card — the size of one raw
fetched image buffer, confirming the mechanism). Fixed (`run_image_evidence_cohort.py`'s
`_run_cohort`): fetch submission is now drip-fed via `_submit_more_fetch`, bounded by the SAME
`queue_depth` knob on both sides of the fetch/compute boundary, plus a secondary fix (a
`fetch_futures` set that was never pruned as futures were consumed — a smaller, ~1%-of-the-effect
leak in the same code path, closed in the same change). The parent process also now logs its own
RSS every progress line, and an optional `--max-rss-mb` makes an excessive parent RSS a clean,
resumable stop instead of relying on the OS OOM-killer. A synthetic repro (tracked, weakref-
observable payloads standing in for real image bytes, no live run) reproduced the mechanism
directly: ~100% of a 400-card synthetic cohort's payloads simultaneously alive at peak with the
un-windowed submission pattern, vs. ~5% (bounded by `queue_depth`) once fetch submission was also
windowed — see the fix's own PR description for the full numbers. Not yet re-run against live prod
at full remainder scale; the next 197k-card invocation is the owner's own authorized run, not part
of this fix.
