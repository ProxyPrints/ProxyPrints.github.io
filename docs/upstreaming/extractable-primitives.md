...
in a file whose *other* top-level imports drag in the vote system). None
...
A row marked `CLEAN` below is a claim: *this file imports nothing from the
fork-only vote system, `CanonicalPrinting`/consensus, or auth modules.*
...
*direct* imports of the exact file(s) listed in a row, one level deep —
it does not walk the import graph transitively. A file that's clean itself
but imports a *different* local file that's entangled (e.g.
...
*semantically* tied to printing identification (even though it has no
fork-only import), is not included in this row's primitive — an extractor
drops that one function and keeps the rest.

**Federation hash tool** — the *code artifact* has zero fork dependencies
...
design is the *hashing recipe* itself (crop box, bleed/trim classification,
...
federation *protocol* (signing scheme, `VoteSource.FEDERATED` gate,
