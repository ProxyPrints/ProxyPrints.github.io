...
12. **Backend test-infra: deterministic snapshot-sequencing pattern**
    (backend L) — a four-line `factory_boy` fixture technique, not a file;
    zero fork-only imports, zero deps, applicable to any project pairing
    `factory_boy` with snapshot testing.
